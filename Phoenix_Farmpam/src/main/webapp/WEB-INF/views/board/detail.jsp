...
<div class="container">
	<c:if test="${dto.prevNum ne 0 }">
		<a href="detail.do?board_idx=${dto.prevNum }&keyword=${encodedK }&condition=${condition }">이전글</a>
	</c:if>
	<c:if test="${dto.nextNum ne 0 }">
		<a href="detail.do?board_idx=${dto.nextNum }&keyword=${encodedK }&condition=${condition }">다음글</a>
	</c:if>
	...
	<table>
		...
		<tr>
			<td colspan="2">
				<div class="content">${dto.board_content }</div>
			</td>
		</tr>
	</table>
	<ul>
				...
				<c:choose>
					...
					<c:otherwise>
						<c:if test="${tmp.comments_idx eq tmp.comments_group }">
							<li id="reli${tmp.comments_idx }">
						</c:if>
						<c:if test="${tmp.comments_idx ne tmp.comments_group }">
							<li id="reli${tmp.comments_idx }" style="padding-left:50px;">
		...
		if(isBottom && !isLoading && !isLast){
			...
			*/
			ajaxPromise("ajax_comment_list.do","get",
					"pageNum="+currentPage+"&board_idx=${dto.board_idx}")
